...
class MoleExcerciseResponse(BaseModel):
    exercice_list: list[MoleExcercise]
...
async def generate_grammar_exercises():
    ...
    try:
        ...
        prompt = """Generate a JSON object with French grammar exercises. Each exercise contains a French sentence with exactly one grammar error, a list that contains all words appearing in the sentence, the answer indicating which word is incorrect, and an explanation of why using that word is wrong.
Rules:
- Each sentence should be no longer than 12 words.
- Ensure there is exactly one grammar error that corresponds to exactly one word.
Format:
{"excercise_list": [{"exercise": "some sentence with exactly one error", "words": ["wordA", "wordB"], "answer": "ansWord", "explanation": "why it is wrong"}]}
Generate exactly 10 exercises following this structure."""
